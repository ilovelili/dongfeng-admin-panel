...
export default () => {
  ...
  return {
    ...
    init(config = {}) {
      ...
      defaultCommands['tlb:rotate-right'] = {
        run: function run(ed) {
          var sel = ed.getSelected();

          if (!sel || !sel.get('draggable')) {
            console.warn('The element is not rotatable');
            return;
          }
          
          var el = sel.getEl();
          var regex = /rotateZ\((-?\d+)deg\)/;
          var match = el.style.transform.match(regex);
          var rotateZ = 0;
          if (match && match.length == 2) {
            rotateZ = parseInt(match[1]);
          }
          var rotateStyle = `rotateZ(${++rotateZ}deg)`;
          console.log(rotateStyle);
          el.style.transform = rotateStyle;
        }
      };

      defaultCommands['tlb:rotate-left'] = {
        run: function run(ed) {
          var sel = ed.getSelected();
